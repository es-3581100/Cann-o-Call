...
## 6. Native membrane truthfulness — bounded vs NOT_IMPLEMENTED

This section is authoritative and overrides any incidental phrasing elsewhere that could be misread as claiming unrestricted native capability. The implemented surface is intentionally bounded; unrestricted forms are explicitly not implemented.

- bounded reference/native membrane: the only implemented native surface is a bounded reference/native membrane in `internal/capability/native/membrane.go` and its `NativeExecutor` wrapper (`internal/capability/native_executor.go`). It is an isolated, typed adapter with no `cgo`, no `plugin.Open`, no `dlopen`/`dlsym` on user-supplied paths.
- pre-allowlisted Go functions only: execution is limited to pre-allowlisted Go functions only. Default allowlist is `libmath.add` and `libstring.upper`; any other library/symbol is rejected with `native_unavailable`. The membrane's Go switch dispatches only those functions; there is no dynamic linking or external `.so`/`.dylib` load.
- unrestricted PureGo/dlopen NOT_IMPLEMENTED: unrestricted PureGo/dlopen NOT_IMPLEMENTED. Claims of arbitrary native extension via PureGo, `dlopen`, `dlsym`, or `plugin` are false for this repository. The codebase contains no `import "github.com/ebitengine/purego"`, no `cgo`, and the grep `purego|dlopen|cgo` is empty except for documentation of their absence.
- arbitrary library loading false: arbitrary library loading false — `Membrane.Invoke` rejects library strings containing `/` or `..` and rejects any library not present in the explicit allowlist map. There is no user-supplied library path, no filesystem scan, no `LD_LIBRARY_PATH` interpretation.
- arbitrary symbol execution false: arbitrary symbol execution false — symbol must be explicitly allowlisted per library (`IsAllowed` check); default `ArbitrarySymbolExecution() bool` returns `false`.
- no arbitrary shell: no arbitrary shell — the membrane performs no shell invocation; the reference process executor (`cap.process.echo`) rejects any non-empty `shell` field (`shell execution not allowed`) and only allows typed `binary`+`args` with an explicit binary allowlist (`echo`, `cat`). No `sh -c <user>` is ever constructed.
- native execution does not gain canonical-state authority: native execution does not gain canonical-state authority — a successful `Membrane.Invoke` or `NativeExecutor.Execute` produces only a `Result{Outputs}` (read-only, `Mutating:false`, `TierPrivileged`). Canonical state mutation still requires `Manager` → `Admission.Authorize` → `Authority.Propose` → `eventlog.Append` with Rust `DurableWriter` ACK. `Executor` has no `Propose` method and cannot advance `transition.Projection` on its own.
- passing shell-bypass protection means arbitrary shell blocked not bypass exists: passing shell-bypass protection means arbitrary shell blocked not bypass exists. A passing test (e.g., `26_no_arbitrary_shell`, `native_unavailable` on non-allowlisted symbol, `ArbitraryLibraryLoading()==false`) is evidence that arbitrary shell/library/symbol execution remains blocked — it does NOT indicate a bypass mechanism exists or that unrestricted execution was enabled.

Classification clarification for §2: the row `PureGo/native execution if added | PRIVILEGED (T3)` refers solely to this bounded membrane form. Unrestricted PureGo/dlopen remains NOT_IMPLEMENTED and must not be inferred as available. Likewise `Arbitrary shell sh -c / dlopen on user path | UNSUPPORTED` remains rejected.
